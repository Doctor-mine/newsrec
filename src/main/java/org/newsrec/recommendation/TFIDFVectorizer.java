...
public class TFIDFVectorizer {
    ...
    public Map<String, Double> buildVector(String document, List<String> corpus) {
        List<List<String>> tokenizedCorpus = corpus.stream()
                .map(preprocessor::tokenize)
                .collect(Collectors.toList());

        Map<String, Double> idfMap = precomputeIDF(tokenizedCorpus);
        return buildVector(document, idfMap);
    }

    public Map<String, Double> buildVector(String document, Map<String, Double> idfMap) {
        List<String> terms = preprocessor.tokenize(document);
    ...
    public Map<String, Double> precomputeIDF(List<List<String>> tokenizedCorpus) {
        Map<String, Long> docCount = new HashMap<>();

        for (List<String> docTokens : tokenizedCorpus) {
            Set<String> unique = new HashSet<>(docTokens);
